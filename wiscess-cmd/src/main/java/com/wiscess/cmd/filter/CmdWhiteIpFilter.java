package com.wiscess.cmd.filter;
...
import com.wiscess.cmd.util.NestUtil;
...
@Slf4j
public class CmdWhiteIpFilter extends OncePerRequestFilter implements OrderedFilter{

	/**
	 * session中设置的可用用户名
	 */
	public static String cmdUsernames="cmd.username";
	public static String cmdEnable="cmd.enable";
	 /**
	   * 白名单IP列表
	   */
	...
	public CmdWhiteIpFilter() {
		...
		log.info("CmdWhiteIpFilter inited.");
	}
	@Value("${cmd.ipurl:http://42.96.168.102:8081/ip.properties}")
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			String strEnable = getSessionAttribute(session,cmdEnable,"false");
	 ...
	 * @return
	 */
	private boolean isViewUser(HttpServletRequest request) {
		HttpSession session=request.getSession();
		//从session中读取指定参数
		String userName=getCurrentUser(session, "NOT_LOGIN_USER");
		String cmdViewUsernames = getSessionAttribute(session,cmdUsernames,"admin");
		return cmdViewUsernames.indexOf(userName)!=-1;
	}
	/**
	 * 获取当前登录用户
	 * @param session
	...
	/**
	 * 取session中的名称，如果不存在，则取默认值
	 * @param session
	 * @param attribute
	 * @param defaultValue
	 * @return
	 */
	private String getSessionAttribute(HttpSession session,String attribute,String defaultValue) {
